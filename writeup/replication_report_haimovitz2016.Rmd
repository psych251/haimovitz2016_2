...
write_csv(qualtrics_data, "./raw_data/qualtrics_data.csv")

##### load coded_responses from raters 1 and 2
coded_response_1 <- read.csv("", header = TRUE, sep = ",")
coded_response_2 <- read.csv("", header = TRUE, sep = ",")

#### Data Exclusion / Filtering
qualtrics_data <- qualtrics_data |>
  mutate(DoNotUse = ifelse(id = , 1, NA)) |> #identify which ids need to be excluded
  filter(is.na(DoNotUse == TRUE)) |> #filter out all the ids that need to be excluded
  select(-c("DoNotUse"))
...
coded_response_double <- coded_response_double |>
  mutate(coded_por = avg(coded_por_1, coded_por_2),
         coded_lor = avg(coded_lor_1, coded_lor_2))
coded_response_double_short <- coded_response_double |>
...
coded_response <- rbind(coded_response_1_single, coded_response_2_single, coded_response_double_short)

#### Prepare Qualtrics Data

##### label each participant with the correct condition
###### NOTE TO SELF: Try to find a way on Qualtrics to record Condition so I don't have to code some of this (still keep the part where I take out raw responses)
qualtrics_data <- qualtrics_data |>
  #take out raw responses that won't be used for analysis
  select(-c("oer_1", "oer_2", "oer_3")) |>
  #temporarily make into tidy data so fidmanip and fiemanip variables can be used to assign condition
  pivot_longer(fidmanip_1:fiemanip_5,
               names_to="item",
               values_to="value") |>
  #add condition; if fidmanip is detected and there is a value for those midmanip variables, then name as debilitating, same for fiemanip
  mutate(condition = ifelse(str_detect(item, "fidmanip_")==TRUE && is.na(value)=FALSE, "debilitating", 
                            ifelse(str_detect(item, "fiemanip_")==TRUE && is.na(value)=FALSE, "enhancing"), NA)) |>
  #change back to un-tidy data
  pivot_wider(names_from = "item",
              values_from = "value")

##### create dataframe with composite scores
data_composite <- qualtrics_data |>
  #format into tidy data
  pivot_longer(competence1:failure6), #1st item in competence scale to last item in failure scale 
               names_to="item", 
               values_to="value") |>
  #reverse score 
  mutate(#add survey name
         measure = ifelse(str_detect(item, "competence_")==TRUE, "competence",
                          ifelse(str_detect(item, "fidmanip_")==TRUE, "biased_fid",
                                 ifelse(str_detect(item, "fiemanip_")==TRUE, "biased_fie",
         ...
         reverse_score = ifelse(item == "failure_1" | item == "failure_2" | item == "failure_3", TRUE, FALSE),
         ...
         value_w_rs = ifelse(reverse_score == TRUE, (reverse.code(-1, rating, mini = 1, maxi = 6)), value)) |>
  group_by(assessment) |>
  summarise(composite_score = mean(value, na.rm=TRUE)) |>
  pivote_wider(names_from="assessment",
               values_from="composite_score")
```
...
icc_por <- icc(coded_response_1$coded_por_1, coded_response_2$coded_por_2) #performance oriented responses
icc_lor <- icc(coded_response_1$coded_lor_1, coded_response_2$coded_lor_2) #learning oriented responses
...
##### Child's Age
summary(lm(data_composite$dem_childage ~ data_composite$failure))
# similar code here for other measures

##### Sex
summary(lm(data_composite$dem_sex ~ data_composite$failure))
# similar code here for other measures

##### Education (SES)
summary(lm(data_composite$dem_education ~ data_composite$failure))
# similar code here for other measures
...
summary(t.test(data_composite$biased_fid, mu = 3.5, alternative = "two.sided"))
summary(t.test(data_composite$biased_fie, mu = 3.5, alternative = "two.sided"))

##### Main Manipulation check
fie_failure <- data_composite |>
  select(c("failure", "condition")) |>
  #get all the failure composites from just FIE condition
  filter(condition == "enhancing") 

fid_failure <- data_composite |>
  select(c("failure", "condition")) |>
  #get all the failure composites from just FIE condition
  filter(condition == "debilitating")

summary(t.test(fie_failure$failure, fid_failure$failure, alternative = "two.sided", var.equal = FALSE)
        
data_manipulation <- data_composite |>
  select(c("failure", "condition")) |>
  group_by(condition) |>
  summarise(
    count = n(),
    mean_failure = mean(failure, na.rm = TRUE),
    sd = sd(failure, na.rm = TRUE), 
    sem_failure = sd / sqrt(count)
  )

fig_manipulation <- ggplot(data_manipulation, aes(x=condiiton, y=mean_failure)) +
  geom_bar(position="dodge", stat="identity") + 
  geom_errorbar(aes(ymin=mean_failure-sem_failure,
                    ymax=mean_failure+sem_failure),
                width=0.1,
...
condition_data <- data_composite |>
  select(c("prolific_id", "condition"))
por_lor_condition_data <- merge(coded_response, condition_data, by = "prolific_id")

por_lor_enhancing_data <- por_lor_condition_data |>
  filter(condition == "enhancing") 

por_lor_debilitating_data <- por_lor_condition_data |>
  filter(condition == "debilitating")

summary(t.test(por_lor_enhancing_data$coded_por, por_lor_debilitating_data$coded_por, alternative = "two.sided", var.equal = FALSE)

por_lor_condition_summary <- por_lor_condition_data |>
  group_by(condition) |>
  summarise(
    count = n(),
...
fig_por_condition <- ggplot(por_lor_condition_summary, aes(x=condiiton, y=mean_por)) +
  geom_bar(position="dodge", stat="identity") + 
...
summary(t.test(por_lor_enhancing_data$coded_lor, por_lor_debilitating_data$coded_lor, alternative = "two.sided", var.equal = FALSE)

fig_lor_condition <- ggplot(por_lor_condition_summary, aes(x=condiiton, y=mean_lor)) +
  geom_bar(position="dodge", stat="identity") + 
