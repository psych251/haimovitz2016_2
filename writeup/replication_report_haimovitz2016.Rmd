...
120 participants (60 mothers and 60 fathers) were recruited via Prolific as planned. 119 of these participants completed the survey, and their responses were saved on Qualtrics. Out of these 119 participants, four participants' responses were excluded, as they were not possible to code due to the following reasons: 1) responses were written in another language (*N* = 1), 2) one of the open-ended responses was missing (*N* = 1), 3) the statements were impossible to interpret due to critical typos or lack of clarity in their response (*N* = 2). The final dataset consisted of responses from 115 participants. 59 of the participants were in the failure-is-debilitating condition and 56 were in the failure-is-enhancing condition.

The final dataset had the following demographic characteristics:

* **Gender:** 49% identifying as woman or female; 50% identifying as man or male; 2% identifying as genderqueer, gender fluid, or non-binary (participants could have chosen more than one category)
* **Education:** 1% had no degrees, 24% had a high school diploma, 55% had a college degree, 19% had a post-graduate degree, and 1% preferred not to answer. 
* **Race/Ethnicity:** 4% Asian, 16% Black/African American, 17% Hispanic, Latino, or Spanish origin, 1% Native Hawaiian or Other Pacific Islander, 63% White, 3% filled in their own option, and 1% preferred not to answer (participants could have chosen more than one category)
...
#create figure
fig_education <- ggplot(metadata, aes(x=dem_education)) +
  geom_bar() +
...
###### Parental Education by Condition
```{r}
#create table
education_condition_count <- metadata |>
  select(dem_education, condition) |>
  mutate(count = 1) |>
  group_by(dem_education, condition) |>
  summarise(N = sum(count)) |>
  kable(col.names = c("Parental Education", "Condition", "N"))
education_condition_count

#create figure
fig_education_condition <- ggplot(metadata, aes(x=dem_education, fill=condition)) +
  ...
  scale_fill_brewer("Condition",
                    labels=c("Failure-Is-Debilitating Condition",
                               "Failure-Is-Enhancing Condition"), 
                    palette = "Dark2")
fig_education_condition
```
...
data_gender <- metadata |>
  select(dem_gender_1:dem_gender_8, condition) |>
...
#create figure
fig_gender <- ggplot(data_gender, aes(x=gender)) +
  geom_bar() +
  xlab("Gender") +
  ylab("N") +
...
data_gender |> 
  ...
  summarise(N = sum(count)) |>
  kable(col.names = c("Gender", "Condition", "N"))

#create figure
fig_gender_condition <- ggplot(data_gender, aes(x=gender, fill=condition)) +
...
data_race_ethnicity <- metadata |>
  select(dem_raceethnicity_1:dem_raceethnicity_8, condition) |>
...
#create figure
fig_race_ethnicity <- ggplot(data_race_ethnicity, aes(x=factor(race_ethnicity, 
                                                               level = c("Asian",
...
###### Race/Ethnicity by Condition
```{r}
#create table
data_race_ethnicity |> 
  group_by(race_ethnicity, condition) |>
  summarise(N = sum(count)) |>
  kable(col.names = c("Race/Ethnicity", "Condition", "N"))

#create figure
fig_race_ethnicity_condition <- ggplot(data_race_ethnicity, aes(x=factor(race_ethnicity,
                                                                         level = c("Asian",
                                                                                   "Black or African American", 
                                                                                   "Hispanic, Latino, or Spanish origin",
                                                                                   "Native Hawaiian or Other Pacific Islander",
...
#create figure
hist_age <- ggplot(metadata, aes(x=dem_childage)) +
...
###### Child's Age By Condition
```{r}
# compute relevant information to generate figure   
data_fig_age_condition <- metadata |>
  group_by(condition) |>
  summarise(
    count = n(),
    mean_age = mean(dem_childage, na.rm = TRUE),
    sd_age = sd(dem_childage, na.rm = TRUE), 
    sem_age = sd_age / sqrt(count)
  )
data_fig_age_condition

fig_age_condition <- ggplot(data_fig_age_condition, aes(x=condition, y=mean_age, color=condition)) +
  geom_point(size=3) +
  geom_errorbar(aes(ymin=mean_age-sem_age,
                    ymax=mean_age+sem_age),
                width=0.1, size=1.2) + 
  geom_jitter(data=metadata, aes(x=condition, y=dem_childage), width = 0.1, height = 0, alpha=0.6) +
  ylab("Child's Age") +
  scale_y_continuous(breaks=seq(0,36,1)) +
  scale_x_discrete("Condition",
                   labels=c("debilitating" = "Failure-is-Debilitating", 
                            "enhancing" = "Failure-is-Enhancing")) +
  theme_classic() +
...
fig_condition_lor <- ggplot(data_coded_response_lor_fig, aes(x=condition, y=mean_lor)) +
  ...
  geom_signif(
    y_position = c(3.5), 
    xmin = c(1.1), xmax = c(1.9),
    annotation = c(paste("p=", round(t_test_lor$p.value, digits = 3))), 
    tip_length = 0.0,
    textsize = 3.5) + 
  theme_classic()
...
##### Was the intended mindset above the midpoint for the failure-is-debilitating condition?
...
data_manip_1_fid_ttest <- t.test(data_manip_1_fid$composite, mu = 3.5, alternative = "two.sided")
data_manip_1_fid_ttest
```

##### Was the intended mindset above the midpoint for the failure-is-enhancing condition?
...
# Was the parents' agreement with each of the intended mindsets above the midpoint?
data_manip_1_fie_ttest <- t.test(data_manip_1_fie$composite, mu = 3.5, alternative = "two.sided")
data_manip_1_fie_ttest
```
...
fig_manip_1 <- ggplot(data_manip_1_fig, aes(x=measure, y=mean, color=measure)) + 
  geom_point(size=3) + 
  geom_errorbar(aes(ymin=mean-sem,
                    ymax=mean+sem),
                width=0.1, size=1.2) +
  ...
  scale_y_continuous(breaks = c(1:6),
                       ...
                       labels = c("Strongly Disagree (1)", "Mostly Disagree (2)", "Slightly Disagree (3)",
                                "Slightly Agree (4)", "Mostly Agree (5)", "Strongly Agree (6)")) +
  ylab("Composite Response of Biased Survey") +
  geom_hline(yintercept=3.5, linetype="dashed") + 
  annotate(geom="text", x=2.48, y=3.7, label="Midpoint") +
  geom_signif(
    y_position = c(6.5, 6.5), 
    xmin = c(0.8, 1.8), xmax = c(1.2, 2.2),
    annotation = c(paste("p=", round(data_manip_1_fid_ttest$p.value, digits = 5)),
                   paste("p=", round(data_manip_1_fie_ttest$p.value, digits = 23))), 
    tip_length = 0.0,
    textsize = 3.5, 
    color = "black") +
  theme_classic() +
...
```{r}
# generate figure  
fig_manip_2 <- ggplot(data_manip_2_fig, aes(x=condition, y=mean_f, color=condition)) +
  geom_point(size=3) + 
  ...
  scale_x_discrete("Condition",
                   labels=c("debilitating" = "Failure-is-Debilitating",
                            "enhancing" = "Failure-is-Enhancing")) +
  ylab('"Failure is Debilitating" Composite Score') +
  expand_limits(y=c(0,7)) +
  coord_cartesian(ylim = c(1, 7)) +
  scale_y_continuous(breaks = c(1:6),
                     # label them with names
  ...
  geom_signif(
    y_position = c(6.5), 
    xmin = c(1.1), xmax = c(1.9),
    annotation = c(paste("p=", format(round(data_manip_2_ttest$p.value, digits = 5), scientific = FALSE))), 
    tip_length = 0.0,
    textsize = 3.5,
    color = "black") +
  labs(caption = '\nNote: The "Failure is Debilitating" values on the y-axes were computed by taking the six responses to \nthe positively- and negatively- worded failure mindset survey questions and creating composite \nscores for each participant. The positively-worded questions that indicated failure is enhancing (e.g., \n"Experiencing failure facilitates my learning and growth.") were reverse-scored. Higher values on the \ny-axes therefore indicate stronger agreement to the idea that failure is debilitating.') +
  theme_classic() +
  theme(legend.position = "none",
...
##### Manipulation Check Follow-Up Analyses

The analyses above reveals a high level of variability in failure mindset even within the conditions. I was therefore curious to ask the following question: 

**Across conditions, is it possible that parents who more strongly believe failure is debilitating react to failure scenarios in 1) less learning-oriented ways and 2) more performance-oriented ways, compared to parents who disagree that failure is debilitating?**

```{r}
#Combine failure mindset composite scores with learning-oriented and performance-oriented responses
coded_lor_por <- metadata |>
  select(c("prolific_id", "coded_por", "coded_lor", "debrief_q2"))
data_manip_change_followup <- merge(coded_lor_por, data_manip_2, by="prolific_id")

#Linear regressions
summary(lm(data_manip_change_followup$coded_lor ~ data_manip_change_followup$composite_f))
summary(lm(data_manip_change_followup$coded_por ~ data_manip_change_followup$composite_f))
```
```{r}
#generate figure
data_manip_change_followup_fig_1 <- data_manip_change_followup |>
  pivot_longer(coded_por:coded_lor,
               names_to="code_type",
               values_to="score")

fig_1_manip_change_followup <- ggplot(data=data_manip_change_followup_fig_1, aes(x=composite_f, y=score)) +
  facet_wrap(~code_type,
             labeller = labeller(code_type = c("coded_lor" = "Learning-Oriented Responses",
                                               "coded_por" = "Performance-Oriented Responses"))) +
  geom_point() +
  geom_smooth() +
  xlab('"Failure is Debilitating" Composite Score') +
  ylab("Number of Responses") +
  ...
  theme_bw()
fig_1_manip_change_followup
```

#### Funnel Debrief Analysis

##### Debrief Question: Are you familiar with the term failure mindset, or growth/fixed mindset?
```{r}
kable(table(metadata$debrief_q2), col.names = c("Answer", "N"))

fig_debrief2 <- ggplot(metadata, aes(x=debrief_q2)) +
  geom_bar() +
  xlab("Are you familiar with the term failure mindset, or growth/fixed mindset?") +
  ylab("N") +
  scale_y_continuous(breaks=seq(0,50,5)) +
  theme_classic() 
fig_debrief2

fig_debrief2_condition <- ggplot(metadata, aes(x=debrief_q2, fill=condition)) +
  geom_bar(position="dodge") +
  xlab("Are you familiar with the term failure mindset, or growth/fixed mindset?") +
  ylab("N") +
...
fig_debrief2_condition
```

##### Debrief Question: Do you think your own mindset impacts your child(ren)'s mindset?

```{r}
data_debrief3 <- metadata |>
  filter(debrief_q2 == "Yes")
kable(table(data_debrief3$debrief_q3), col.names = c("Answer", "N"))

fig_debrief3 <- ggplot(data_debrief3, aes(x=debrief_q3)) +
  geom_bar() +
  xlab("Do you think your own mindset impacts your child(ren)'s mindset?") +
  ylab("N") +
...
##### Did parents' knowledge of mindset research have any effect on their performance-oriented and learning-oriented responses? 

```{r fig.width=10, fig.height = 6} 
data_funneldebrief <- metadata |>
  pivot_longer(coded_por:coded_lor,
               names_to = "code_type", 
               values_to = "response")

data_funneldebrief_stats <- data_funneldebrief |>
  group_by(debrief_q2, code_type) |>
  summarise(count = n(),
            mean = mean(response, na.rm = TRUE),
            sd = sd(response, na.rm = TRUE), 
            sem = sd / sqrt(count),)

data_funneldebrief_cond_stats <- data_funneldebrief |>
  group_by(debrief_q2, condition, code_type) |>
  summarise(count = n(),
            mean = mean(response, na.rm = TRUE),
            sd = sd(response, na.rm = TRUE), 
            sem = sd / sqrt(count),)

fig_funneldebrief <- ggplot(data=data_funneldebrief_stats, aes(x=debrief_q2, y=mean)) +
  facet_wrap(~code_type,
             labeller = labeller(code_type = c("coded_lor" = "Learning-Oriented Responses",
                                               "coded_por" = "Performance-Oriented Responses"))) +
  geom_point(colour="black", size=3) +
  geom_errorbar(aes(ymin=mean-sem, ymax=mean+sem), width=.2, colour="black", size=1) +
  geom_jitter(data = data_funneldebrief, aes(x = debrief_q2, y = response, color=condition), alpha=0.4,
              position = position_jitterdodge()) +
  geom_point(data=data_funneldebrief_cond_stats, aes(x=debrief_q2, y=mean, color=condition), size=3, 
             position=position_dodge(width=1)) +
  geom_errorbar(data=data_funneldebrief_cond_stats, aes(ymin=mean-sem, ymax=mean+sem, color=condition), 
                width=.2, size=1, 
                position=position_dodge(width=1)) +
  scale_color_brewer(labels=c("Failure-Is-Debilitating Condition",
                              "Failure-Is-Enhancing Condition"), 
                     palette = "Dark2") +
  xlab("Are you familiar with the term failure mindset, or growth/fixed mindset?") +
  ylab("Number of Responses") +
...
##### Is it possible that parents' knowledge of mindset research affected their response to the failure mindset survey used in the manipulation check?

```{r}
debrief_q2 <- metadata |> 
  select(c("prolific_id", "debrief_q2"))
data_manip_2_w_debrief <- merge(debrief_q2, data_manip_2, by="prolific_id")

fig_manip_2_w_debrief <- ggplot(data_manip_2_fig, aes(x=condition, y=mean_f)) +
  geom_point(size=3) + 
  geom_errorbar(aes(ymin=mean_f-sem_f,
                    ymax=mean_f+sem_f),
                width=0.1, size=1.2) +
  geom_jitter(data=data_manip_2_w_debrief, 
              aes(x=condition, y=composite_f, color=debrief_q2),
              width = 0.1, height = 0, alpha=0.6) +
  scale_x_discrete("Condition",
                   labels=c("debilitating" = "Failure-is-Debilitating",
                            "enhancing" = "Failure-is-Enhancing")) +
  ...
  theme(legend.position = "top",
        plot.caption = element_text(hjust = 0)) +
  scale_color_discrete("Familiarity with Mindset Research")

fig_manip_2_w_debrief
```
...
The key analysis of interest was an unpaired, two-tailed t test comparing performance-oriented responses between the failure-is-debilitating condition and failure-is-enhancing condition. The original authors reported the following result: 

> “Parents who were induced to hold a failure-is-debilitating mind-set were more likely to react with concerns about their child’s performance and lack of ability, t(131) = 3.246, p < .001, ηp2 = .075 … compared with those who were induced to hold a failure-is-enhancing mind-set.”

The replication study showed similar trends to this original finding. The effect was not as large and there was no statistical difference in the number of performance-oriented responses between the two conditions, t(`r round(t_test_por$parameter)`) = `r round(t_test_por$statistic, digits=4)`, p = `r round(t_test_por$p.value, digits=4)`, ηp2 = .031. However, I think my replication was rather successful, given that my sample size was smaller. 

### Commentary

The exploratory analysis comparing the difference in the learning-oriented responses between the two conditions was promising. Although the difference was not significant, the finding showed similar trends to the original study. 

The exploratory analyses also provided some insight as to why my results showed less of a difference between the two conditions, compared to the original study. 

1) In line with the original study, each of the conditions' average response to the biased surveys differed from the midpoint and was above it, indicating agreement. However, plotting individual participants' responses to the biased survey revealed that there were a lot of parents in the failure-is-debilitating condition who were below the midpoint and actually disagreed with the biased survey questions. Furthermore, the average response within the failure-is-debilitating condition was slightly higher in the original study, indicating that there was slightly more agreement to the failure-is-debilitating statements in the original study.

2) In line with the original study, parents in the failure-is-debilitating condition reported more of a failure-is-debilitating mindset at the end of the paradigm. However, there was a lot of individual variability especially within the failure-is-debilitating group. Furthermore, the average numerical value for failure mindset was 3.19, which corresponds to the answer choice: "slightly disagree." In other words, those in the failure-is-debilitating condition reported that on average, they "slightly disagree" to the idea that failure is debilitating. 

3) Further analyses on the funnel debrief question also hint at the possibility that past knowledge of mindset research may have affected the results. 

Overall, these exploratory analyses provide some reason to believe that many parents seem to inherently believe that failure is enhancing, and that it can be hard to induce a different belief. This phenomenon may especially be prevalent more recently, given that the mindset literature has become overly popularized. 

More generally, I learned that there is a level of subjectivity in coding open-ended responses, making these types of studies hard to replicate. The authors provided a detailed coding scheme, which was crucial in helping me code the responses. In addition to documenting detailed procedures around coding, sharing the raw response data and the scores could be helpful as well. This information would be especially useful in providing more detailed examples of how the raw responses were coded.

Taken together, the replication study was rather successful. Coding open-ended responses can be subjective and prone to error. Furthermore, the exploratory analyses showed that failure mindsets were hard to induce in the first place. Despite these factors that went against successfully replicating my study, I was able to find promising, marginal differences between the two conditions. 