...
120 parents were recruited to achieve 80% power and to account for some participants whose data may not be usable. Parents were recruited through a crowdsourcing platform called Prolific. The participant pool on Prolific was filtered by parental status to specifically reach parents.
...
Furthermore, I decided to include a funnel debrief at the end to make sure that the effects I observed were due to priming rather than explicit consistency biases. The funnel debrief included questions to assess whether the participants were trying to be consistent with their own responses. These questions were not a part of the original study. 
...
The exploratory analysis aimed to replicate other findings reported in the original study. [add more details here]
...
##### Gender
kable(table(qualtrics_data$dem_gender), col.names = c("Gender", "N"))
...
alpha_f <- psych::alpha(data_raw_f)
alpha_f
...
alpha_c <- psych::alpha(data_raw_c)
alpha_c
